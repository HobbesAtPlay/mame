...
void menu::draw(UINT32 flags)
{
	...
	if (ui().show_fps_counter())
	{
		ui().draw_text_full(container(), machine().video().speed_text().c_str(), 0.0f, 0.0f, 1.0f,
				ui::text_layout::RIGHT, ui::text_layout::WORD, mame_ui_manager::OPAQUE_, RGB_T__WHITE, RGB_T__BLACK, nullptr, nullptr);
	}
	...
	if (selected_subitem_too_big)
	{
		...
		ui().draw_text_full(container(), pitem.subtext.c_str(), 0, 0, visible_width * 0.75f,
			ui::text_layout::RIGHT, ui::text_layout::WORD, mame_ui_manager::NONE, RGB_T__WHITE, RGB_T__BLACK, &target_width, &target_height);
...
void menu::draw_text_box()
{
	...
	ui().draw_text_full(container(), text, 0, 0, 1.0f - 2.0f * UI_BOX_LR_BORDER - 2.0f * gutter_width,
		ui::text_layout::LEFT, ui::text_layout::WORD, mame_ui_manager::NONE, RGB_T__WHITE, RGB_T__BLACK, &target_width, &target_height);
...
void menu::draw_background()
{
	...
	if (ui().options().use_background_image() && m_global_state->bgrnd_bitmap() && m_global_state->bgrnd_bitmap()->valid())
		container().add_quad(0.0f, 0.0f, 1.0f, 1.0f, RGB_T__WHITE, m_global_state->bgrnd_texture(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
}
