...
void menu_select_launch::draw_star(float x0, float y0)
{
	...
	container().add_quad(x0, y0, x1, y1, RGB_T__WHITE, m_cache->star_texture(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA) | PRIMFLAG_PACKABLE);
}
...
float menu_select_launch::draw_icon(int linenum, void *selectedref, float x0, float y0)
{
	...
	if (m_icons_bitmap[linenum] != nullptr && m_icons_bitmap[linenum]->valid())
		container().add_quad(x0, y0, x1, y1, RGB_T__WHITE, m_icons_texture[linenum], PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
...
std::string menu_select_launch::arts_render_common(float origx1, float origy1, float origx2, float origy2)
{
	...
	for (int x = FIRST_VIEW; x < LAST_VIEW; x++)
	{
		...
		ui().draw_text_full(container(),
				...
				ui::text_layout::CENTER, ui::text_layout::TRUNCATE, mame_ui_manager::NONE, RGB_T__WHITE, RGB_T__BLACK,
				&text_length, nullptr);
...
void menu_select_launch::draw_snapx(float origx1, float origy1, float origx2, float origy2)
{
	...
	if (snapx_valid())
	{
		...
		// apply texture
		container().add_quad(x1, y1, x2, y2, RGB_T__WHITE, m_cache->snapx_texture(), PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
	}
}
