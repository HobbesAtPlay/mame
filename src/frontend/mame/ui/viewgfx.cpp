...
static void palette_handler(mame_ui_manager &mui, render_container &container, ui_gfx_state &state)
{
	...
	for (auto ch : title)
	{
		container.add_char(x0, y0, chheight, mui.machine().render().ui_aspect(), RGB_T__WHITE, *ui_font, ch);
	...
	for (x = 0; x < state.palette.columns; x += 1 + skip)
	{
		...
		container.add_char(x0 + 0.5f * (cellwidth - chwidth), y0, chheight, mui.machine().render().ui_aspect(), RGB_T__WHITE, *ui_font, "0123456789ABCDEF"[x & 0xf]);

		// if we're skipping, draw a point between the character and the box to indicate which
		// one it's referring to
		if (skip != 0)
			container.add_point(x0 + 0.5f * cellwidth, 0.5f * (y0 + chheight + cellboxbounds.y0), UI_LINE_WIDTH, RGB_T__WHITE, PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
	}
	...
	for (y = 0; y < state.palette.columns; y += 1 + skip)
		...
		if (state.palette.offset + y * state.palette.columns < total)
		{
			...
			if (skip != 0)
				container.add_point(0.5f * (x0 + cellboxbounds.x0), y0 + 0.5f * cellheight, UI_LINE_WIDTH, RGB_T__WHITE, PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
			...
			for (x = 4; x >= 0; x--)
			{
				...
				container.add_char(x0, y0 + 0.5f * (cellheight - chheight), chheight, mui.machine().render().ui_aspect(), RGB_T__WHITE, *ui_font, buffer[x]);
			}
		}
...
static void gfxset_handler(mame_ui_manager &mui, render_container &container, ui_gfx_state &state)
{
	...
	for (x = 0; x < xcells; x += 1 + skip)
	{
		...
		// one it's referring to
		if (skip != 0)
			container.add_point(x0 + 0.5f * cellwidth, 0.5f * (y0 + chheight + boxbounds.y0 + 3.5f * chheight), UI_LINE_WIDTH, RGB_T__WHITE, PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
	}
	...
	for (y = 0; y < ycells; y += 1 + skip)
		...
		if (info.offset[set] + y * xcells < gfx.elements())
		{
			...
			y0 = boxbounds.y0 + 3.5f * chheight + (float)y * cellheight;
			if (skip != 0)
				container.add_point(0.5f * (x0 + boxbounds.x0 + 6.0f * chwidth), y0 + 0.5f * cellheight, UI_LINE_WIDTH, RGB_T__WHITE, PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
	...
	container.add_quad(cellboxbounds.x0, cellboxbounds.y0, cellboxbounds.x1, cellboxbounds.y1,
						RGB_T__WHITE, state.texture, PRIMFLAG_BLENDMODE(BLENDMODE_ALPHA));
